...
import { saveDemoProgress } from "@/actions/save-demo-progress" // Importa la nueva Server Action
...
export function DemoWizard({ initialData }: DemoWizardProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    estimatedEmployees: "", // Nuevo campo para la cantidad exacta de empleados
  ...
  const calculateDv = (T: string) => {
    let M = 0,
      S = 1
    for (; T; T = Math.floor((T as any) / 10) as any) S = (S + (T % 10) * (9 - (M++ % 6))) % 11
  ...
  const handleNext = async () => {
    ...
    // Guardar progreso antes de avanzar o finalizar
    const saveResult = await saveDemoProgress({
      formData: formData,
      currentStep: currentStep,
      isFinalSubmission: currentStep === 4, // Es la sumisión final si estamos en el paso 4
    })
  ...
  // Función para calcular el precio (placeholder)
