...
import { Send, Loader2 } from "lucide-react" // Importa Loader2
  ...
  return (
    <Card className="w-full max-w-md h-[600px] flex flex-col rounded-xl shadow-2xl border-none">
      ...
      <CardContent className="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50">
        ...
        {isLoading && ( // Muestra el indicador de carga cuando la IA está pensando
      ...
      <form onSubmit={handleSubmit} className="p-4 border-t bg-white flex items-center space-x-2">
        <Input
          ...
          disabled={isLoading} // Deshabilita el input mientras la IA está cargando
        />
        <Button
          type="submit"
          className="rounded-full w-10 h-10 p-0 flex items-center justify-center bg-blue-600 hover:bg-blue-700"
          disabled={isLoading || !input.trim()} // Deshabilita el botón si está cargando o el input está vacío
        >
